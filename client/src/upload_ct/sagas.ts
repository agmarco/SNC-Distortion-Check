...
export function* uploadCtToS3(action: Action<actions.IUploadCtToS3Payload>): any {
    const errorMsg = `Your file could not be uploaded. Please try again,
            or contact CIRS support if this problem persists.`;

    yield put(formActions.setPending('uploadCt', true));

    const getS3UrlResponse = yield api.signS3((action.payload as actions.IUploadCtToS3Payload).file);
    ...
    if (getS3UrlResponse.ok) {
        ...
        postData.append('file', (action.payload as actions.IUploadCtToS3Payload).file);
        ...
        if (uploadToS3Response.ok) {
            ...
            yield put(formActions.setPending('uploadCt', false));
